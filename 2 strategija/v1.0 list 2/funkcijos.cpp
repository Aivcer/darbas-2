...
void nuskaitymas(int &k, list <duomenys> &d)
{
    ...
    d.resize(k);
    k=0;
    for( list<duomenys>::iterator i=d.begin(); i!=d.end(); i++ )
    {
...
/*void rasymas(int k, list <duomenys>& d)
{
...
void rusiavimas(int k, list <duomenys> &d)
{
    auto start = std::chrono::high_resolution_clock::now();
    list <duomenys> nus;
    list<duomenys>::iterator it = d.begin();
    while (it != d.end())
    {
        if ( (0.4*vidurkis((*it).C) + 0.6*(*it).er) < 5 )
        {
            nus.push_back((*it));
            it = d.erase(it);
        }
    ...
    ofstream out1("Nuskriaustukai" + to_string(k) + ".txt");
    ofstream out2("Protuoliai" + to_string(k) + ".txt");
    for (list<duomenys>::iterator i=nus.begin(); i!=nus.end(); ++i)
